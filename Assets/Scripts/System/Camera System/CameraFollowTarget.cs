...
public class CameraFollowTarget : MonoBehaviour
{
    ...
    [Space, Header("타겟 사이 거리 조절")]
    public float targetDistance = 10f;

    public float distance = 10f;

    [Space, Header("줌 속도 조절")]
    ...
    private void LateUpdate()
    {
        ...
        else if (targets != null && targets.Count > 0)
        {
            ...
            if (!reduceCameraMotion && targets.Count == 1 && targets[0].gameObject != null && targets[0].gameObject.CompareTag("Player Camera Bone"))
            {
            ...
            if (reduceCameraMotion && Vector3.Distance(vector, CurrentPosition) < 0.05f)
            {
